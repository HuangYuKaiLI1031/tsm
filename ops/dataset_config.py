...
ROOT_DATASET = 'E:\Project\TSM\\temporal-shift-module\dataset'  # '/data/jilin/'


def return_ucf101(modality):
    filename_categories = 'label\\classInd.txt'
    if modality == 'RGB':
        root_data = ROOT_DATASET + '/image'
        filename_imglist_train = 'label/ucf101_rgb_train_split_1.txt'
        filename_imglist_val = 'label/ucf101_rgb_val_split_1.txt'
...
def return_dataset(dataset, modality):
    ...
    print(file_imglist_train)
    file_imglist_val = os.path.join(ROOT_DATASET, file_imglist_val)
    if isinstance(file_categories, str):
        file_categories = os.path.join(ROOT_DATASET, file_categories)
        with open("E:\Project\TSM\\temporal-shift-module\\dataset\label\\classInd.txt") as f:
            lines = f.readlines()
