...
class TSN(nn.Module):
    ...
    args:
        ...
        base_model:基础模型之后的TSN模型以此基础来修改
        new_length:视频取帧的起点，rgb为1，光流为5
        consensus_type:聚合函数的选择
        before_softmax:布尔类型，判断是否进行softmax
        dropout:正则化
        img_feature_dim:图片的特征维度，代码中未使用到
        crop_num:数据集修改的类别
        partial_bn:是否部分bn
        print_spec:打印信息
        pretrain:布尔型，判断是否加载预训练的模型
        is_shift:布尔型，判断是否加入shift操作
        shift_div:表示shift操作的比例倒数
        shift_place:判断标志，当为"blockres"表示嵌入了残差模块，当为"block"时表示嵌入了直链模块。
        fc_lr5:学习率优化策略中对第五个全连接进行参数调整的判断标志。
        temporal_pool:表示是否在时间维度进行池化降维，相应的第2、3和4层num_segment减半
        non_local:布尔型，判断是否加入non_local网络模块
    """
    ...
    def _construct_diff_model(self, base_model, keep_rgb=False):
        ...
        # 获取卷积层的参数 weights
        params = [x.clone() for x in conv_layer.parameters()]
        # size(64,3,7,7) (output_channels,input_channels/阻塞通道数量,kernel_size[0],kernel_size[1])
        # Conv2d.weight url:https://pytorch.org/docs/stable/generated/torch.nn.Conv2d.html#torch.nn.Conv2d   
        kernel_size = params[0].size()
        if not keep_rgb:
            # 重新设计weights的形状。主要修改第二个参数即输入通道的大小
            new_kernel_size = kernel_size[:1] + (3 * self.new_length,) + kernel_size[2:]
            # tensor.data仅获取tensor的数据不获取grad
            # tensor.mean(dim,keepdim) dim求平均值的维度（该维度大小变为1），keepdim是否与原tensor的size保持一致 （64，3，7，7）-->(64,1,7,7)
            # tensor.expand(shape)扩展维度。原tensor中必须有一个维度的大小为1 shape：扩展之后的尺寸
            # tensor.contigous()简单理解为复制，得到一个与原tensor相同的tensor但是改变其中一个的数据对另一个tensor没有影响
            new_kernels = params[0].data.mean(dim=1, keepdim=True).expand(new_kernel_size).contiguous()
        else:
            new_kernel_size = kernel_size[:1] + (3 * self.new_length,) + kernel_size[2:]
