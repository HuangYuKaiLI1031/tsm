...
root = r"E:/Project/TSM/temporal-shift-module/dataset/image"      # 抽帧后的图片存放目录文件夹，用于写到txt文件中在构建数据集的时候读取

def parse_ucf_splits():
    class_ind = [x.strip().split() for x in open(r'D:\DOWNLOAD\\UCF101\\label/classInd.txt')]       # 类别txt
    ...
    splits = []
    for i in range(1, 4):
        train_list = [line2rec(x) for x in open(r'D:\DOWNLOAD/UCF101/label/trainlist{:02d}.txt'.format(i))]       # 训练集txt
        test_list = [line2rec(x) for x in open(r'D:\DOWNLOAD/UCF101/label/testlist{:02d}.txt'.format(i))]          # 测试集txt
...
def build_split_list(split_tuple, frame_info, split_idx, shuffle=False):
    ...
    def build_set_list(set_list):
        ...
        for item in set_list:
            ...
            # buffer = frame_dir.split('\\')
            # print(buffer)
            # frame_dir = os.path.join(root,frame_dir.split('\\')[-2],frame_dir.split('\\')[-1])
...
f_info = parse_directory(r"E:/Project/TSM/temporal-shift-module/dataset/image")    # 存放抽帧后的图片

out_path = r"E:/Project/TSM/temporal-shift-module/dataset/label"      # 标签路径
dataset = "ucf101"
